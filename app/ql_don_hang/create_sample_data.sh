#!/bin/bash

# Script tạo dữ liệu mẫu cho API Đơn Hàng
# Chạy: chmod +x create_sample_data.sh && ./create_sample_data.sh

BASE_URL="https://boney-unspoiled-thesis.ngrok-free.dev/api/don-hang"

echo "========================================"
echo "TẠO DỮ LIỆU MẪU CHO ĐƠN HÀNG"
echo "========================================"
echo ""

# Đơn hàng 1: iPhone 15 Pro Max - Placed
echo "Đang tạo đơn hàng 1: iPhone 15 Pro Max..."
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2024-01-15",
    "customerName": "Nguyễn Văn A",
    "totalAmount": 59980000,
    "status": "Placed",
    "address": "123 Đường Lê Lợi, Quận 1, TP.HCM",
    "phone": "0901234567",
    "products": [
      {
        "name": "iPhone 15 Pro Max",
        "model": "256GB - Titan Tự Nhiên",
        "quantity": 2,
        "price": 29990000
      }
    ]
  }'
echo ""
echo "✅ Đơn hàng 1 đã được gửi"
echo ""
sleep 1

# Đơn hàng 2: Samsung Galaxy S24 Ultra - Confirmed
echo "Đang tạo đơn hàng 2: Samsung Galaxy S24 Ultra..."
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2024-01-16",
    "customerName": "Trần Thị B",
    "totalAmount": 31990000,
    "status": "Confirmed",
    "address": "456 Đường Nguyễn Huệ, Quận 3, TP.HCM",
    "phone": "0912345678",
    "products": [
      {
        "name": "Samsung Galaxy S24 Ultra",
        "model": "512GB - Đen",
        "quantity": 1,
        "price": 31990000
      }
    ]
  }'
echo ""
echo "✅ Đơn hàng 2 đã được gửi"
echo ""
sleep 1

# Đơn hàng 3: MacBook Pro M3 - Shipping
echo "Đang tạo đơn hàng 3: MacBook Pro M3..."
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2024-01-17",
    "customerName": "Lê Văn C",
    "totalAmount": 45000000,
    "status": "Shipping",
    "address": "789 Đường Trần Hưng Đạo, Quận 5, TP.HCM",
    "phone": "0923456789",
    "products": [
      {
        "name": "MacBook Pro M3",
        "model": "14 inch - 16GB RAM - 512GB SSD",
        "quantity": 1,
        "price": 45000000
      }
    ]
  }'
echo ""
echo "✅ Đơn hàng 3 đã được gửi"
echo ""
sleep 1

# Đơn hàng 4: iPad Pro - Delivered
echo "Đang tạo đơn hàng 4: iPad Pro..."
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2024-01-10",
    "customerName": "Phạm Thị D",
    "totalAmount": 25000000,
    "status": "Delivered",
    "address": "321 Đường Võ Văn Tần, Quận 10, TP.HCM",
    "phone": "0934567890",
    "products": [
      {
        "name": "iPad Pro 12.9",
        "model": "256GB - WiFi + Cellular",
        "quantity": 1,
        "price": 25000000
      }
    ]
  }'
echo ""
echo "✅ Đơn hàng 4 đã được gửi"
echo ""
sleep 1

# Đơn hàng 5: Apple Watch Ultra 2 - Cancelled
echo "Đang tạo đơn hàng 5: Apple Watch Ultra 2..."
curl -X POST "$BASE_URL" \
  -H "Content-Type: application/json" \
  -d '{
    "date": "2024-01-12",
    "customerName": "Hoàng Văn E",
    "totalAmount": 18000000,
    "status": "Cancelled",
    "cancelDate": "2024-01-13",
    "cancelReason": "Khách hàng đổi ý, không muốn mua nữa",
    "address": "555 Đường Cách Mạng Tháng 8, Quận Tân Bình, TP.HCM",
    "phone": "0945678901",
    "products": [
      {
        "name": "Apple Watch Ultra 2",
        "model": "Titanium - GPS + Cellular",
        "quantity": 2,
        "price": 9000000
      }
    ]
  }'
echo ""
echo "✅ Đơn hàng 5 đã được gửi"
echo ""

echo "========================================"
echo "HOÀN THÀNH TẠO DỮ LIỆU MẪU"
echo "========================================"
echo ""
echo "Bạn có thể kiểm tra bằng cách:"
echo "1. Chạy app và xem danh sách đơn hàng"
echo "2. Gọi API GET: curl $BASE_URL"
echo ""
